...
当前版本已经能完整跑通实际入库主链，但仍属于开发与实战测试版本，不应被误读为已经发布的稳定安装版。软件项目最擅长的事情之一，就是让“能跑”和“可以放心交付”看起来像同一句话。
...
- 图库明细只读展示已经提交的图集与图片，不把数据库变成给用户看的 SQL 控制台。
...
这仍然是不可撤销的危险操作，只是尽量不让危险操作顺便进化成数据事故。
...
除非该命令在 clean Windows 环境真实通过，否则不要把正式发布状态写成已完成。文档不会因为写得自信就自动变成测试证据，可惜很多项目似乎持不同意见。
