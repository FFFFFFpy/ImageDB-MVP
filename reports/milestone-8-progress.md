...
## 2026-07-04: Library-root disconnect recovery gate

### 实现内容

- 新增真实 PostgreSQL + 真实文件系统故障注入测试，覆盖图库根挂载点消失后的 Recovery 行为。
- commit 在 `AfterDbWrite` 中断后，测试将 library root 重命名为 offline 路径，模拟已挂载共享目录断开。
- Recovery 在断连状态下必须暂停，保持 transaction 为 `staging`，父 import run 保持 `recovery_required`，并写入可诊断的 `last_error`。
- library root 恢复后，同一 transaction 可由 Recovery 继续推进并最终收敛到 `source_archived`。
...
- `cargo fmt --manifest-path apps/desktop/src-tauri/Cargo.toml`：passed。
- `IMAGEDB_POSTGRES_BIN=D:\MyProjects\Agent\ImageDB-MVP\.local\db-tools\postgresql-18.4\pgsql\bin cargo test --manifest-path apps/desktop/src-tauri/Cargo.toml --features fail-injection,real-db-tests --lib fail_injection_library_root_disconnect_pauses_then_recovers -- --ignored --test-threads=1`：1 passed。

### 实际运行结果

- 断连时 Recovery 返回 `final_state=staging`、`recovered=false`，消息包含 `recovery paused`。
- 数据库中 transaction 保持 `staging`，import run 保持 `recovery_required`，未产生完成状态。
- 恢复 library root 后，Recovery 成功完成发布、DB commit 和 source archive。
