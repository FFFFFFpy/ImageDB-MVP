...
- [x] 全部测试、Clippy和Release构建通过（本次 `format:check` / `typecheck` / `test:unit` / `rust:test` / `rust:clippy` / `rust:test:real` / `build` / `release:verify-artifacts` / `release:install-gate` 均通过）。
