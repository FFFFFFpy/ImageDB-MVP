...
- [x] 精确重复组使用最低 UUID 代表边，候选数量从平方级降为线性。
...
- `pnpm rust:test`：229 通过、0 失败、3 个真实数据库用例按设计忽略。
- `pnpm rust:test:real`：24 个真实测试组、103 个用例全部通过，耗时 650.6 秒；门禁包含 V2 缺失哈希拒绝与旧索引移除的真实迁移测试。

算法回归夹具全部为程序生成的小图片，不包含大规模压力测试。覆盖文件/像素精确匹配、500 张精确重复的线性代表边、透明像素、EXIF、哈希长度、归一化距离、并列粗距离的精细变换择优、缩放、JPEG 质量、亮度、水印、主体位移、相邻动作、明显不同图片、索引构建/批量 upsert/增量/移除/稳定排序/稳定截断及非 V2 排除。
