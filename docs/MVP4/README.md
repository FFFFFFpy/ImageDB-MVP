...
- 解码器在读取图片头并校验像素总数后使用显式 limits；产品上限为 5 亿像素，1 亿像素以上全局只允许一个大图解码任务。
- EXIF identity 路径直接复用 `into_rgba8` 的 owned buffer，不复制第二份全尺寸 RGBA。
...
- 大图支持不改变 Fingerprint V2 的哈希定义、缩放滤镜、变换集合或重复判定阈值。
