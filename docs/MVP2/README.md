...
- frozen plan 可选择默认关闭的“移动选中源文件（无备份）”；该模式将选中图片原子隔离到同目录持久化临时路径后，对隔离文件逐一校验并删除，不删除源图集目录、sidecar、嵌套文件、排除图片或隔离后在原路径新建的文件。隔离证据冲突时不自动恢复原名，保留隔离文件及其完整路径供人工处理。
...
0016 新增持久化审核组、源文件模式和逐文件清理操作日志，不回填旧的未完成审核任务。0017 为每条源文件清理操作持久化同目录隔离路径，使校验对象与最终删除对象绑定；恢复会以单次数据库更新为 0016 在途清理行补写唯一隔离路径并规范化旧 operation state，旧 `verifying` 文件重新进入隔离验证，旧 `removing` 仅在源文件已不存在时视为删除完成。
