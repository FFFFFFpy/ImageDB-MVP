...
2026-07-14 在提交 `69a2e96` 补齐 PostgreSQL 技术探针的生命周期保护。`probe_postgres` 和 `run_all_probes` 进入 PostgreSQL 初始化前取得 `ProbeManagedDatabase` 独占操作租约；Scan、Commit、Recovery 或 External migration 活跃时立即拒绝，拒绝不会注销原任务。探针租约存续期间也拒绝新任务启动，关闭任务启动与探针初始化之间的双向 TOCTOU 窗口。

Probes 页面读取同一 guard 状态：状态未确认、加载失败或已有关键任务时，数据库探针与“运行全部探针”保持禁用，并提供错误与重试；图片指纹和隔离文件事务探针不受影响。若前端 guard 缓存过期而后端最终拒绝，页面会显示真实错误。Settings / Probes 继续共享父导航视觉激活，但只有实际 Settings 页面声明 `aria-current="page"`。

| 命令                                       | 本次补充复验结果                                           |
| ------------------------------------------ | ---------------------------------------------------------- |
| `pnpm format:check`                        | 通过，Prettier 与 `cargo fmt --check` 均无差异             |
| `pnpm typecheck`                           | 通过，`tsc --noEmit` 无错误                                |
| `pnpm test:unit`                           | 15 个文件、136 项通过，0 失败                              |
| `pnpm --filter @imagedb/desktop build:web` | 通过，231 modules；JS 380.53 KB，CSS 174.03 KB             |
| `pnpm rust:test`                           | 232 通过、0 失败、3 个 real test 按设计忽略                |
| `pnpm rust:clippy`                         | 通过，`--all-targets --all-features -D warnings`           |
| `pnpm rust:test:real`                      | 102 项真实 PostgreSQL / 文件系统测试通过，0 失败，733.4 秒 |
| `pnpm check`                               | 通过，完整串行复验类型、单测、格式、Clippy 与 Rust 单测    |

定向回归包含 6 个 Rust 探针租约测试、5 个 Layout 导航语义测试和 4 个 Probes 页面测试。无活动任务时 guard 会执行探针并在结束后释放租约；真实 PostgreSQL 生命周期与 partial-init recovery 继续由本次 `rust:test:real` 验证。本次没有新增 schema migration，没有重新执行 release exe / NSIS 构建，也没有另行启动 Tauri UI 人工回归；这些未执行项不借用历史结果宣称已复验。
