...
- [x] Recovery 完成 M3 迁移。
- [x] 设置与技术诊断完成 M3 迁移。
- [x] PostgreSQL 技术探针和“运行全部探针”服从后端生命周期互斥；活跃 Scan / Commit / Recovery / External migration 期间 fail closed，指纹与隔离文件事务探针仍可用。
- [x] 图库概览可进入只读图库明细，并展开查看已提交图集的图片清单。
...
- [x] `abandoned` / `invalidated` 是 M3.8 唯一授权的状态语义扩展；其余工作流与文件事务语义保持不变。
...
- [x] 图库明细明确区分 loading、error、empty 与成功数据，且只提供只读交互。
...
- [x] Library / Probes 可保持父级导航视觉激活，但只有实际 Dashboard / Settings 页面声明 `aria-current="page"`。
- [x] 高频进度更新不会持续打断屏幕阅读器。

## 7. 性能

- [x] 1,000 个图集或 10,000 个候选的页面策略已验证，不一次渲染全部重内容。
- [x] 图库明细每批最多读取 50 个图集，展开后每批最多读取 24 张图片。
...
以下勾选项对应 [`VALIDATION.md`](VALIDATION.md) 已记录的历史执行批次，不自动证明后续修复后的当前 HEAD。每轮修复必须在实际执行命令后另行记录结果；未执行的命令不得沿用旧勾选项宣称通过。
...
- Windows 100% / 150% 系统缩放人工抽样；它们不属于本轮 M3.8 审查修复的完成门禁，也不得作为待完成阻塞项。
