...
当前已锁定 `animal-island-ui@1.2.2`，完成 React 19 / Vite 6 / Tauri 2 的兼容性验证，并建立 ImageDB 语义 token、本地 UI 适配层、基础组件测试和开发期视觉夹具。全部生产页面已完成 M3 迁移：审核左右图语义由测试锁定，计划与提交保持两个步骤，Commit 只读同一份 frozen plan，Recovery 明确区分可恢复、冲突与终态事务。设置 About 已展示作者、CC BY-NC 4.0 和个人非商业用途声明。计划图集和图片分别按 50 个、24 张分批挂载。审查修复后，显式 workflow run ID 会贯穿扫描、审核与提交；Commit 还会校验用户确认时看到的 `plan_hash`。用户后续明确授权的 M3.8 扩展已增加“入库事务开始前撤销 frozen plan”和“只读图库明细”：撤销保留失效计划与审核审计，任何文件事务证据出现后即 fail closed；图库只展示已提交图集，并按 50 个图集、24 张图片渐进读取。完整自动门禁、真实 PostgreSQL/文件系统测试、压力夹具、全页 WCAG 自动审计、Tauri release/NSIS 构建、真实 Tauri UI 主链以及分析/Commit 中断恢复证据见 [`VALIDATION.md`](VALIDATION.md)。当前机器以 Windows 125% 系统缩放完成实测；M3.7 仅因 100% 与 150% 系统缩放尚未人工切换签字而保持打开。
...
- frozen plan 撤销只能发生在文件事务创建前；计划以 `invalidated` 状态保留审计，审核决定与源快照不删除，已开始的 Commit / Recovery 不允许通过撤销绕过。
