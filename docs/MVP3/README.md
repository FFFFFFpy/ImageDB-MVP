...
- 整次未入库导入只能在文件事务创建前撤销；import run 进入 `abandoned`、计划以 `invalidated` 状态保留审计，审核决定与源快照不删除，任务不再作为可继续工作流出现；已开始的 Commit / Recovery 不允许通过撤销绕过。
