...
文档基线：已建立
视觉方向：已确定
代码实现：M3.0–M3.2 已完成，M3.3 待开始
人工验收：未开始
```

当前已锁定 `animal-island-ui@1.2.2`，完成 React 19 / Vite 6 的兼容性验证，并建立 ImageDB 语义 token、本地 UI 适配层、基础组件测试和开发期视觉夹具。应用壳与工作台已完成迁移，后端全部八种 `next_action` 均有参数化文案映射测试；1440×900、960×720 和 720×720 浏览器截图无关键操作裁切。Tauri 实际窗口和完整主链验收留在对应页面任务包与 M3.7。
