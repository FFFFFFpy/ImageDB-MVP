...
文档基线：已建立
视觉方向：已确定
代码实现：M3.0–M3.4 已完成，M3.5 待开始
人工验收：未开始
```

当前已锁定 `animal-island-ui@1.2.2`，完成 React 19 / Vite 6 的兼容性验证，并建立 ImageDB 语义 token、本地 UI 适配层、基础组件测试和开发期视觉夹具。应用壳、工作台、新建导入、分析进度与图片审核工作台已完成迁移；审核左右图的保留语义由常量与单元测试锁定，叠加比较、缩放平移和键盘决策继续保留。后端全部八种 `next_action` 均有参数化文案映射测试，原生目录选择 IPC 已通过 Rust 编译、215 项 Rust 测试与 Clippy。Tauri 完整主链验收留在 M3.7。
