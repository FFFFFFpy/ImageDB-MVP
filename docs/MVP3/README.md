...
视觉方向：已确定
代码实现：M3.0–M3.7 已完成
验收状态：自动门禁、真实主链、Tauri release 运行与 WebView 导航均已通过
```

当前已锁定 `animal-island-ui@1.2.2`，完成 React 19 / Vite 6 / Tauri 2 的兼容性验证，并建立 ImageDB 语义 token、本地 UI 适配层、基础组件测试和开发期视觉夹具。全部生产页面已完成 M3 迁移：审核左右图语义由测试锁定，计划与提交保持两个步骤，Commit 只读同一份 frozen plan，Recovery 明确区分可恢复、冲突与终态事务。设置 About 已展示作者、CC BY-NC 4.0 和个人非商业用途声明。计划图集和图片分别按 50 个、24 张分批挂载。M3.7 已通过 78 项前端单测、212 项常规 Rust 测试、99 项真实 PostgreSQL/文件系统测试、Clippy、性能门禁、Tauri release/NSIS 构建、隔离进程启动及真实 WebView2 导航。完整证据见 [`VALIDATION.md`](VALIDATION.md)。
