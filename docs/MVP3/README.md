...
```text
除用户明确授权的 M3.8 受限扩展外，不改变现有导入、审核、冻结计划、Commit 与 Recovery 业务语义，
...
文档基线：已建立
视觉方向：已确定
代码实现：M3.0–M3.8 已落地，当前处于审查缺陷修复与验证收口
验收状态：历史验证证据已记录；本轮结果只在命令实际执行后补录，Windows 100% / 150% 不作为本轮门禁
```

当前已锁定 `animal-island-ui@1.2.2`，完成 React 19 / Vite 6 / Tauri 2 的兼容性验证，并建立 ImageDB 语义 token、本地 UI 适配层、基础组件测试和开发期视觉夹具。全部生产页面已完成 M3 迁移：审核左右图语义由测试锁定，计划与提交保持两个步骤，Commit 只读同一份 frozen plan，Recovery 明确区分可恢复、冲突与终态事务。设置 About 已展示作者、CC BY-NC 4.0 和个人非商业用途声明。计划图集和图片分别按 50 个、24 张分批挂载。审查修复后，显式 workflow run ID 会贯穿扫描、审核与提交；Commit 还会校验用户确认时看到的 `plan_hash`。用户后续明确授权的 M3.8 扩展已增加“入库事务开始前撤销整次导入”和“只读图库明细”：撤销会使 frozen plan 失效、将 import run 记为 `abandoned`，并回到工作台的新建导入入口；审核决定、源快照与计划历史继续保留审计，任何文件事务证据出现后即 fail closed。图库只展示已提交图集，并按 50 个图集、24 张图片渐进读取。`abandoned` / `invalidated` 是 M3.8 唯一授权的状态语义扩展；除此之外，原工作流、frozen plan、Commit、文件事务与 Recovery 语义保持不变。历史自动门禁、真实 PostgreSQL/文件系统测试、压力夹具、全页 WCAG 自动审计、Tauri release/NSIS 构建、真实 Tauri UI 主链以及分析/Commit 中断恢复证据见 [`VALIDATION.md`](VALIDATION.md)，这些历史数字不自动代表后续修复后的当前 HEAD。Windows 100% / 150% 系统缩放可继续作为兼容性抽样，但不属于本轮完成门禁，也不再阻塞 M3 收口。
...
| [`VALIDATION.md`](VALIDATION.md)                   | 按执行批次记录自动、真实环境与桌面验收 |
...
- 整次未入库导入只能在文件事务创建前撤销；import run 进入 `abandoned`、计划以 `invalidated` 状态保留审计，审核决定与源快照不删除，任务不再作为可继续工作流出现；已开始的 Commit / Recovery 不允许通过撤销绕过。这是 M3.8 唯一授权的状态语义扩展。
- 图库明细只读已提交事实，查询经 command → service → repository，React 不直接访问数据库或文件系统。
- M3 不修改数据库 schema、migration、匹配算法或文件事务协议。若 UI 审查暴露既有契约缺口，必须作为独立、可测试的后端契约修复提交；当前 `plan_hash` 确认校验即按此规则实现。
