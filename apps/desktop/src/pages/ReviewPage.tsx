...
interface ReviewPageProps {
  ...
  onGoScan?: (importRunId: string) => void;
  onStartImport?: () => void;
...
export function ReviewPage({
  ...
  const importRunId = initialImportRunId;
    ...
    return (
      <div className="review-page review-page--m3">
        ...
        <EmptyState
          ...
          action={
            <Button onClick={() => (onStartImport ? onStartImport() : onNavigate('scan'))}>
              开始导入
            </Button>
          }
        />
      ...
      {allResolved && !analysisComplete && (
        <StatusBanner
          ...
          actions={
            <Button
              onClick={() =>
                onGoScan ? onGoScan(importRunId) : onNavigate('scan')
              }
            >
              继续分析
            </Button>
